...
pub struct Contract {
    ...
    nft_count: Counter,
    comment_map: LookupMap<i8, Vec<String>>,
    charge_map: LookupMap<i8, u128>,
}
...
impl Contract {
    ...
    pub fn nft_mint(
        ...
        let token_id: i8 = self.nft_count.get_num();
        let token_id_str = self.nft_count.get_num().to_string();
        self.comment_map.insert(&token_id, &vec![]);
        self.charge_map.insert(&token_id, &0);
        self.tokens.mint(token_id_str, receiver_id, Some(token_metadata))
    }

    #[payable]
    pub fn set_comment( &mut self, _token_id: i8, _comment: String) {
        self.comment_map.insert(&_token_id, &vec![_comment]);
    }     
      


    #[payable]
    pub fn set_charge( &mut self, _token_id: TokenId, _charge: u128) {
        // todo assert the caller is the contract itself
        Promise::new(_token_id).transfer(_charge as u128);
    }

    pub fn get_comments( &mut self, token_id: i8) -> Vec<String> {
    ...
    pub fn get_charge( &mut self, token_id: i8) -> u128 {
