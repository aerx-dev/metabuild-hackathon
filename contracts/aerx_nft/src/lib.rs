...
pub struct Contract {
    ...
    commentMap: LookupMap<u8, Vec<String>>,
    chargeMap: LookupMap<u8, u128>,
}
...
impl Contract {
    ...
    pub fn new(owner_id: ValidAccountId, metadata: NFTContractMetadata) -> Self {
        ...
        Self {
            ...
            commentMap: LookupMap::new(b"comment".to_vec()),
            chargeMap: LookupMap::new(b"charge".to_vec())
        }
    }
    ...
    pub fn nft_mint(
        ...
        self.commentMap.insert(&token_id, &vec![]);
        self.chargeMap.insert(&token_id, &0);
        self.tokens.mint(token_id_str, receiver_id, Some(token_metadata))
    }

    #[payable]
    pub fn setComment( &mut self, token_id: u8, comment: String) {
        // update the commentMap with insert
    }

    #[payable]
    pub fn setCharge( &mut self, token_id: u8, charge: u128) {
        // assert the caller is the contract itself
        // update the chargeMap with insert. First get the current value, add the charge, insert.
    }

    pub fn getComments( &mut self, token_id: u8) -> Vec<String> {
        // return all comments for that ID
        match self.commentMap.get(&token_id){
            Some(x)=>x, // vector that contains all memos
            None=>vec![] //else this will return an empty vector
        }
    }

    pub fn getCharge( &mut self, token_id: u8) -> u128 {
        // return the charge for that ID
        match self.chargeMap.get(&token_id){
            Some(x) => x, // vector that contains all memos
