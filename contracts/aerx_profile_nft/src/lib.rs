...
use near_sdk::{
    env, near_bindgen, ext_contract, AccountId, BorshStorageKey, PanicOnDefault, Promise, PromiseOrValue,
};
...
impl Contract {
    ...
    #[init]
    pub fn new_default_meta(owner_id: ValidAccountId, _aex_tokenId: ValidAccountId) -> Self {
    ...
    #[init]
    pub fn new(owner_id: ValidAccountId, metadata: NFTContractMetadata, _aex_tokenId: ValidAccountId) -> Self {
    ...
    pub fn nft_mint(
        ...
        assert!(self.tokens_per_owner[receiver_id].is_empty(), "You already minted a profile-NFT!");
        let amount: Balance = env::attached_deposit();
        assert!(AEXToken::get_min_balance(&self.aex_tokenId.to_string(), 0, 1_000) < amount);
        // Collect premie
        assert!(AEXToken::collect_premie(&self.aex_tokenId.to_string(), receiver_id, amount, 1_000), "Something went wrong fetching your premie!");
